...
import CREATE_ENTITY_TYPE_MUTATION from '../mutations/CREATE_ENTITY_TYPE_MUTATION';

test('create entityType', async (t) => {
  ...
  const createResult = await mutate({
    mutation: CREATE_ENTITY_TYPE_MUTATION,
    variables,
  });

  t.equal(createResult.errors, undefined, 'should not throw an error');
  t.deepEqual(
    omit(['id'], createResult.data.createEntityType.entityType),
    {
      name: randomName,
    },
    'should return the created entityType',
  );
